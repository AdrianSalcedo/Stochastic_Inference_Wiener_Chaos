Gibb_OU_exact=function(X,nbri,delta,beta,lambda,K,M,Nrow)
{
  ...
  for(i in 2:(K+M))
  {
    ...
    alphas_aux[i]=pos_alpha(X,nbri,delta,alphas_aux[i-1],beta,Nrow)
    alphas_aux_norm[i]=pos_alpha_norm(X,nbri,delta,alphas_aux_norm[i-1],beta,Nrow)
    if(i>K){
      j=i-K
  ...
  # ProbMTrunc <- Prob_Movil(alphas_aux)
  # ProbMNorm <- Prob_Movil(alphas_aux_norm)
...
pos_alpha=function(X,nbri,delta,alpha,beta,Nrow)
{
  cb=comp_Bridge(X,nbri,delta,alpha,beta)
  n=length(X)
  Kv=Ks(cb,n,delta,nbri,lambda,beta,alpha)
  int=numeric(n)
  alphat <- (0-Kv[2]/(2*Kv[1]))/(sqrt(1/Kv[1]))
  #mut <- Kv[2]/(2*Kv[1]) + sqrt(1/Kv[1])* dnorm(alphat,0,1)/(1-pnorm(alphat,0,1))
  #sdt <- (1/Kv[1])*(1 + alphat*dnorm(alphat,0,1)/(1-pnorm(alphat,0,1)) - (dnorm(alphat,0,1)/(1-pnorm(alphat,0,1)))^2)
  #print(Kv[1])
  #print(Kv[2])
  #print(Kv[2]/(2*Kv[1]))
...
####
pos_alpha_norm=function(X,nbri,delta,alpha,beta,Nrow)
{
  cb=comp_Bridge(X,nbri,delta,alpha,beta)
  n=length(X)
  Kv=Ks(cb,n,delta,nbri,lambda,beta,alpha)
...
comp_Bridge=function(X,nbri,delta,alpha,beta)
{
  ...
  k=length(X)-1
  Nrow <- 1
  cb=matrix(0,nrow=Nrow,ncol=k*(nbri)+1)
  for(s in 1:Nrow){
    for(i in 1:k)
    {
      ini=(i-1)*nbri+1
      fin=i*nbri+1
      cb[s,ini:fin]= Exact_OU(X[i],X[i+1],delta/nbri,nbri,alpha,beta)
    }
  }
  cbm <-colMeans(cb)
  return(cbm)
...
# Prob_Movil<-function(Data)
# {
#   ln <- length(Data)
#   vec_means <- mat.or.vec(1,ln)
#   for(i in 1:ln) 
#   {
#     vec_means[i] <-mean(Data[1,i])
#   }
#   return(vec_means)
# }
