...
pos_beta=function(Xd,nb,deltad,beta,alpha,Nrow,Nbm,z1)
{
  ...
  for (i in 2:1000){
    ...
    while(Cond){
      int = numeric(n)
      z2 = rgamma(1,shape =(n-1)/2+1,scale=2/Kv[1])
      #z2 = rtruncnorm(1,a = 0,b=Inf, mean = Kv[2]/(2*Kv[1]),sd = sqrt(1/(2*Kv[1])))
      print(c(z1,z2))
      Weigth = exp(-(lambda)*(sqrt(1/z2)-sqrt(1/z1))+(Kv[2]/2)*(sqrt(z2^2)-sqrt(z1^2)))
      Prt = min(1,Weigth)
      ut = runif(1,0,1)
      Cond = is.na(ut<=Prt)
      #print(c(i,Prt,z1,z2))
...
Ks=function(cb,n,delta,nb,lambda,alpha,beta)
{
  ...
  K2 = (1/2)*alpha^2*int_path(C2-C5,delta)[nc-1]
